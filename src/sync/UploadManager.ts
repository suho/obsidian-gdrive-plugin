...
export class UploadManager {
	...
	async pushFile(path: string): Promise<'created' | 'updated' | 'skipped'> {
		...
		if (this.isPathExcluded(normalizedPath, file.stat.size)) {
			return 'skipped';
		}
	...
	async pushRename(oldPath: string, newPath: string): Promise<boolean> {
		...
		if (this.isPathExcluded(normalizedNewPath, file.stat.size)) {
			return false;
		}
	...
	private async collectLocalFileStates(): Promise<LocalFileState[]> {
		...
		for (const file of files) {
			if (this.isPathExcluded(file.path, file.stat.size)) {
				continue;
			}
	...
	private isPathExcluded(path: string, sizeBytes: number): boolean {
		return isExcluded(
			path,
